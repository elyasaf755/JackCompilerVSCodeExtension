...
exports.execute = async (args) => {
    ...
    const currFileUrl = vscode.window.activeTextEditor.document.uri.fsPath

    function getCurrDir(currFileUrl){
      if (currFileUrl.includes("\\.vscode")){
        return currFileUrl.split("\\.vscode")[0]
      }

      return currFileUrl.split(/\\\w+.jack/g)[0]
    }

    const currentDirectory = getCurrDir(currFileUrl)
    const compilerPath = currentDirectory + '\\.vscode\\tools';
    ...
    try {
        ...
        if (ans.trim() === 'Compiling \"' + currentDirectory + '\" '.trim()){
